...
let eval_add arg =
  let rec aux arg = match arg with
    ...
    | Cons(Atom a, cdr) -> (int_of_string a) + (aux cdr)
    | Cons(_, _) -> raise (Error "can only add atoms")
  in let k = (aux arg) in Atom (string_of_int k)

let eval_sub arg1 arg2 = match (arg1, arg2) with
  |(Atom a, Atom b) -> Atom (string_of_int((int_of_string a) - (int_of_string b)))
  |(_, _) -> raise (Error "can only sub atoms")

let eval_mul arg1 arg2 = match (arg1, arg2) with
  |(Atom a, Atom b) -> Atom (string_of_int((int_of_string a) * (int_of_string b)))
  |(_, _) -> raise (Error "can only mul atoms")

(** comparison functions **)

let eval_le arg1 arg2 = match (arg1, arg2) with
  |(Atom a, Atom b) -> Atom (string_of_bool((int_of_string a) > (int_of_string b)))
  |(_, _) -> raise (Error "can only compare atoms")

let eval_gr arg1 arg2 = match (arg1, arg2) with
  |(Atom a, Atom b) -> Atom(string_of_bool((int_of_string a) < (int_of_string b)))
  |(_, _) -> raise (Error "can only compare atoms")

let eval_leq arg1 arg2 = match (arg1, arg2) with
  |(Atom a, Atom b) -> Atom( string_of_bool((int_of_string a) < (int_of_string b) || (int_of_string a) = (int_of_string b)))
  |(_, _) -> raise (Error "can only compare atoms")

let eval_geq arg1 arg2 = match (arg1, arg2) with
  |(Atom a, Atom b) -> Atom( string_of_bool((int_of_string a) > (int_of_string b) || (int_of_string a) = (int_of_string b)))
  |(_, _) -> raise (Error "can only compare atoms")
